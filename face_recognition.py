from  tkinter import*
from  tkinter import ttk,messagebox
from PIL import Image,ImageTk 
import pyttsx3
import mysql.connector
from time import strftime
from datetime import datetime
import cv2 
import os
import numpy  as np

class Face_Recognition_sys:
    def __init__(self,root):
        self.root=root  
        self.root.geometry('1530x790+0+0')  
        self.root.title("Face Recognition") 

          #top heading 
        title_label=Label(self.root,text="FACE RECOGNITION",font =("times new roman",35,"bold"),justify="center",bg = "purple",fg="white")
        title_label.place(x=0,y=0,width=1410,height=70)

        #main frame
        main_frame=Frame(self.root,bd=5,bg="white")
        main_frame.place(x=0,y=75,width=1410,height=620)

          # 1st img 
        img_top=Image.open(r"img\\bg_face.jpg")
        img_top=img_top.resize((900,670),Image.ANTIALIAS) 
        self.img_top=ImageTk.PhotoImage(img_top)

        first_label=Label(self.root,image=self.img_top)
        first_label.place(x=5,y=75,width=900,height=670)

        #button
        traindata_btn=Button(main_frame,text="RECOGNISE FACE",command=self.face_recog,font=("times new roman",18,"bold"),bg="purple",fg="white")
        traindata_btn.place(x=1000,y=280,width=230,height=50)

         
    #========================face recognition ==========================================

    def mark_attendance(self,i,r,n,d):
        with open("attendance.csv","r+",newline="\n") as f:
            myDatalist=f.readlines()
            name_list=[]
            for line in myDatalist:
                entry=line.split((","))
                name_list.append(entry[0])
            if((i not in name_list) and (r not in name_list) and (n not in name_list) and (d not in name_list)):
                now=datetime.now()
                d1=now.strftime("%d/%m/%H")
                dtString=now.strftime("%H:%M:%S")
                f.writelines(f"\n{i},{r},{n},{d},{dtString},{d1},Present")


    

    #========================face recognition ==========================================
    def face_recog(self):
        def draw_boundry(img ,classifier1,scaleFactor,minNeighbors,color,text,classifier):
            gray_image=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
            features=classifier1.detectMultiScale(gray_image,scaleFactor,minNeighbors)

            coordinates=[]
            
            for(x,y,w,h) in features:
                cv2.rectangle(img,(x,y),(x+w,y+h),(0,255,0),3)
                id,predict=classifier.predict(gray_image[y:y+h,x:x+w])
                confidence=int((100*(1-predict/300)))

                conn=mysql.connector.connect(host="localhost",user="root",password="MYSQL123",database="face_db")
                my_cursor=conn.cursor()
                
                #fetching name from db
                my_cursor.execute("select Name from student where  Student_id="+str(id))
                n=my_cursor.fetchone()
                n="+".join(n)
                
                #fetching  roll no. from db
                my_cursor.execute("select Roll from student where  Student_id="+str(id))
                r=my_cursor.fetchone()
                r="+".join(r)

                #fetching  dep no. from db
                my_cursor.execute("select Department from student where  Student_id="+str(id))
                d=my_cursor.fetchone()
                d="+".join(d)

                #fetching  ID from db
                my_cursor.execute("select Student_id from student where  Student_id="+str(id))
                i=my_cursor.fetchone()
                i="+".join(i)

                
                if confidence>77:
                    cv2.putText(img,f"Student ID:{i}",(x,y-75),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),2)
                    cv2.putText(img,f"Roll:{r}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),2)
                    cv2.putText(img,f"Name:{n}",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),2)
                    cv2.putText(img,f"Department:{d}",(x,y-5),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),2)
                    self.mark_attendance(i,r,n,d)

                else:
                    cv2.rectangle(img,(x,y),(x+w,y+h),(0,0,255),3)
                    cv2.putText(img,"Unknown Face",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(255,255,255),3)

                coordinates=[x,y,w,h]
            return coordinates
        
        def recognize(img,classifier,faceCascade):
            coordinates=draw_boundry(img,faceCascade,1.1,10,(255,25,255),"Face",classifier)
            return img
        
        faceCascade=cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
        classifier=cv2.face.LBPHFaceRecognizer_create()
        # classifier=cv2.face.LBPHFaceRecognizer_create()

        classifier.read("classifier.xml")

        video_cap=cv2.VideoCapture(1)

        while True:
            ret,img=video_cap.read()
            img=recognize(img,classifier,faceCascade)
            cv2.imshow("welcome to face Recognition",img)
            
            if cv2.waitKey(1)==13:
                break
        video_cap.release()
        cv2.destroyAllWindows()



if __name__=="__main__":
    root=Tk()
    obj=Face_Recognition_sys(root)
    root.mainloop()